...
======================
Automated tests for the meal prediction pipeline.
...
def small_df():
    """Generate a tiny 2-student, 2-week dataset for fast tests."""
...
def trained_artefacts(engineered_df, tmp_path_factory):
    """Train a minimal model and return (model, encoders, feature_cols, threshold)."""
    ...
    threshold = float(np.percentile(probas, 85))   # top 15% → chosen
...
class TestDataLoader:
    def test_shape(self, small_df):
        # 10 students × 7 days × 3 meals × ~6 items × 2 weeks
    ...
    def test_one_choice_per_meal_sitting(self, small_df):
        """Each (student, week, day, meal_type) should have at most 1 choice=1."""
        chosen = small_df[small_df['choice'] == 1]
        counts = chosen.groupby(['week', 'student_id', 'day', 'meal_type']).size()
        assert (counts <= 1).all(), "More than one item chosen per meal sitting"
    ...
    def test_hist_freq_no_leakage(self, engineered_df):
        """Week 1 day Mon hist_freq should be 0 for all rows (no prior history)."""
        w1_mon = engineered_df[(engineered_df['week'] == 1) & (engineered_df['day'] == 'Mon')]
        assert (w1_mon['hist_freq'] == 0).all()
...
class TestPredict:
    def test_validate_inputs_passes(self, trained_artefacts):
        ...
        validate_inputs(encoders, 'Mon', 'Breakfast', 'Eggs')  # should not raise
    ...
    def test_predict_weekly_shape(self, trained_artefacts):
        ...
        # 7 days × 3 meal types = 21 rows
        assert len(df) == 21

    def test_predict_weekly_diversity(self, trained_artefacts):
        """No meal_type should have the same item predicted every single day."""
        model, encoders, feature_cols, threshold = trained_artefacts
        df = predict_weekly(model, encoders, feature_cols,
                             student_id=1, week=2, threshold=threshold)
        for meal_type in config.MEAL_TYPES:
            items = df[df['meal_type'] == meal_type]['menu_item'].tolist()
            # With diversity constraint, should NOT be 7 identical items
            unique_count = len(set(items))
            assert unique_count > 1, (
                f"All 7 days predicted the same '{items[0]}' for {meal_type} — "
                "diversity constraint not working"
            )

    def test_diversity_penalty_applied(self):
        """apply_diversity should lower probability of already-predicted items."""
        items = [
        ...
        already = {'Ugali Beef'}
        result  = apply_diversity(items, already, penalty=0.5)
